...
void ImageColors::generateDominantColor(const std::vector<QRgb> &samples)
{
    ...
    const bool darkTheme = qGray(qGuiApp->palette().window().color().rgb()) < 192;
    const auto &wallpaperColors = WallpaperColors::fromBitmap(samples);
    const auto &colorScheme = ColorScheme(wallpaperColors, darkTheme, Style::VANILLA);

    m_accentColor = QColor(colorScheme.accentColor());
}
