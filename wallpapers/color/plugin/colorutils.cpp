...
{
void colorToHSL(QRgb color, std::array<double, 3> &outHsl)
{
    RGBToHSL(qRed(color), qGreen(color), qBlue(color), outHsl);
}

void RGBToHSL(int r, int g, int b, std::array<double, 3> &outHsl)
{
    const double rf = r / 255.0;
    const double gf = g / 255.0;
    const double bf = b / 255.0;

    const double max = std::max(rf, std::max(gf, bf));
    const double min = std::min(rf, std::min(gf, bf));
    const double deltaMaxMin = max - min;
    ...
    if (max == min) {
        ...
        if (max == rf) {
            h = std::fmod((gf - bf) / deltaMaxMin, 6.0);
        } else if (max == gf) {
        ...
        s = deltaMaxMin / (1.0 - std::abs(2 * l - 1));
    }

    h = std::fmod(h * 60, 360.0);
    if (h < 0) {
        h += 360;
    }

    outHsl[0] = std::clamp(h, 0.0, 360.0);
    outHsl[1] = std::clamp(s, 0.0, 360.0);
    outHsl[2] = std::clamp(l, 0.0, 360.0);
}
...
QRgb CAMToColor(double hue, double chroma, double lstar)
{
