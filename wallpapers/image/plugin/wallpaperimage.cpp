...
static QImage loadPackage(const TransientImageOptions &options)
{
    ...
    if (imageReader.format() == "svg" || imageReader.format() == "svgz") {
        ...
        return svgRender.image(svgRender.size().toSize().scaled(requestedSize, Qt::KeepAspectRatioByExpanding));
...
{
    QImageReader reader(urlToLocalFileOrQrc(options.source));
...
QSGNode *TransientImage::updatePaintNode(QSGNode *oldNode, UpdatePaintNodeData *)
{
    ...
    QRectF sourceRect;
    QRectF targetRect;
    QSGTexture::WrapMode horizontalWrapMode = QSGTexture::ClampToEdge;
    QSGTexture::WrapMode verticalWrapMode = QSGTexture::ClampToEdge;
    ...
    case FillMode::Tile: {
        const QSizeF nativeSize = size() * devicePixelRatio;
        const qreal repetitionsHorizontally = nativeSize.width() / textureSize.width();
        const qreal repetitionsVertically = nativeSize.height() / textureSize.height();

        const int sourceWidth = std::round(repetitionsHorizontally * textureSize.width());
        const int sourceHeight = std::round(repetitionsVertically * textureSize.height());
        const int sourceX = std::floor(0.5 * (textureSize.width() - sourceWidth));
        const int sourceY = std::floor(0.5 * (textureSize.height() - sourceHeight));

        horizontalWrapMode = QSGTexture::Repeat;
        verticalWrapMode = QSGTexture::Repeat;
        sourceRect = QRectF(sourceX, sourceY, sourceWidth, sourceHeight);
        targetRect = QRectF(QPoint(0, 0), size());
        break;
    }
    ...
    case FillMode::TileHorizontally: {
        ...
        sourceRect = QRectF(sourceX, 0, sourceWidth, textureSize.height());
        targetRect = QRectF(QPoint(0, 0), size());
        break;
    }
    ...
    case FillMode::TileVertically:
        ...
        verticalWrapMode = QSGTexture::Repeat;
        sourceRect = QRectF(0, sourceY, textureSize.width(), sourceHeight);
        targetRect = QRectF(QPoint(0, 0), size());
        break;
    }
    ...
    if (!node) {
        node = new TransientImageNode();
    }

    node->setSourceRect(sourceRect);
    node->setTargetRect(targetRect);
    node->setFiltering(QSGTexture::Linear);
    node->setHorizontalWrapMode(horizontalWrapMode);
    node->setVerticalWrapMode(verticalWrapMode);
