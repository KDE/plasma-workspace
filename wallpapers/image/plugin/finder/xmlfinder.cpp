...
QList<WallpaperItem> XmlFinder::parseXml(const QString &path, const QSize &targetSize)
{
    ...
    // Used in ScreenshotRole
    for (auto &item : results) {
        if (!item.slideshow.data.empty()) {
            // Slideshow preview
            for (const auto &sItem : std::as_const(item.slideshow.data)) {
                item.previewImageList.append(sItem.file);
            }
        } else {
            // Adaptive wallpaper preview
            if (!item.filename.isEmpty() && !item.filename.endsWith(QLatin1String(".xml"), Qt::CaseInsensitive)) {
                item.previewImageList.append(item.filename);
            }
            if (!item.filename_dark.isEmpty() && !item.filename_dark.endsWith(QLatin1String(".xml"), Qt::CaseInsensitive)) {
                item.previewImageList.append(item.filename_dark);
            }
        }
    }
