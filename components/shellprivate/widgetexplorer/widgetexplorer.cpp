...
{
    const QString p = QLatin1String(PLASMA_RELATIVE_DATA_INSTALL_DIR) + "/plasmoids/"_L1 + pluginName;
...
void WidgetExplorer::uninstall(const QString &pluginName)
{
    static const QString packageRoot =
        QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + u'/' + QLatin1String(PLASMA_RELATIVE_DATA_INSTALL_DIR) + "/plasmoids/"_L1;
    ...
    if (!job->package().isValid() && !pluginName.isEmpty()) {
        QDir dir(packageRoot + u'/' + pluginName);
        if (dir.exists()) {
