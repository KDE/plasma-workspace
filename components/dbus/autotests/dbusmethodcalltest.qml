...
TestCase {
    ...
    property DBus.dbusReply reply
    property DBus.DBusPendingReply pendingReply

    function initTestCase() {
        verify(!root.reply.isValid);
        verify(root.reply.error.isValid);
        compare(root.reply.error.message.length, 0);
        compare(root.reply.error.name.length, 0);
        compare(typeof root.reply.value, "undefined");
    }
    ...
    function test_call(data) {
        ...
        root.reply = DBus.SessionBus.call(msg);
        verify(root.reply.isValid);
        compare(root.reply.error.message.length, 0);
        compare(root.reply.value, data.replyArgument);
        compare(root.reply.values, [data.replyArgument]);
    }

    function test_asyncCall_data() {
        return test_call_data();
    }
    ...
    function test_asyncCall(data) {
        ...
        // Test DBusPendingReply -> DBusReply ctor
        root.reply = root.pendingReply;
        verify(root.reply.isValid);
        compare(root.reply.value, data.replyArgument);
        compare(root.reply.values, [data.replyArgument]);
    }
    ...
    function test_ping() {
        ...
        root.reply = DBus.SessionBus.call(msg);
        verify(root.reply.isValid);
        verify(!root.reply.value);
        compare(root.reply.values.length, 0);
    }

    function test_wrong_signautre() {
        const msg = {
            "service": root.service,
            "path": root.path,
            "iface": root.iface,
        ...
        root.reply = DBus.SessionBus.call(msg);
        verify(!root.reply.isValid);
        compare(root.reply.error.name, "org.freedesktop.DBus.Error.InvalidArgs");
    }

    function test_invalid_signautre() {
        const msg = {
            "service": root.service,
            "path": root.path,
            "iface": root.iface,
