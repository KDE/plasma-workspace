...
void KLookAndFeelManifest::writeLayout(const QDir &contentsDirectory)
{
    ...
    QFile layoutFile(layoutsDirectory.absoluteFilePath(QStringLiteral("org.kde.plasma.desktop-layout.js")));
    if (layoutFile.open(QIODevice::WriteOnly)) {
        layoutFile.write(desktopLayout().toUtf8());
        layoutFile.close();
    } else {
...
void KLookAndFeelManifest::writePreview(const QDir &contentsDirectory)
{
    ...
    const QImage fullPreview(preview());
