...
void DBusMenuView::setRoot(DBusMenuModel *model, const QPersistentModelIndex &index)
{
    if (d->model != model) {
        if (d->model) {
            ...
            disconnect(d->model, &QAbstractItemModel::rowsRemoved, this, &DBusMenuView::onRowsRemoved);
        ...
        if (model) {
            ...
            connect(model, &QAbstractItemModel::rowsRemoved, this, &DBusMenuView::onRowsRemoved);
...
void DBusMenuView::buildSubTree(QMenu *menu, const QModelIndex &index)
{
    if (const int rowCount = d->model->rowCount(index)) {
        ...
        for (const QAction *action : actions) {
            if (action->property("placeholder").toBool()) {
                return;
            }
        }

        // No actions, add a placeholder so QMenu::popup() doesn't quit.
...
void DBusMenuView::onRowsRemoved(const QModelIndex &parentIndex, int first, int last)
{
