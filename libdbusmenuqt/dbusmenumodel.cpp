...
void DBusMenuModel::updateSubTree(DBusMenuModelItem *item, const DBusMenuLayoutItem &rawItem)
{
    updateItem(item, rawItem.properties);
    ...
    for (int targetRow = 0; targetRow < rawItem.children.size(); ++targetRow) {
        ...
        if (auto childItem = findItemById(rawChildItem.id)) {
            ...
            updateSubTree(childItem, rawChildItem);
...
{
    QDBusPendingReply<uint, DBusMenuLayoutItem> call = d->interface->GetLayout(id, d->prefetchSize, QStringList());
    QDBusPendingCallWatcher *watcher = new QDBusPendingCallWatcher(call, this);
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this, id](QDBusPendingCallWatcher *watcher) {
        ...
        if (DBusMenuModelItem *item = findItemById(id)) {
            updateSubTree(item, reply.argumentAt<1>());
        }
    });
}
