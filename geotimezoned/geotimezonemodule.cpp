...
void KdedGeoTimeZonePlugin::setGeoTimeZone(const QByteArray &geoTimeZoneId)
{
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this, geoTimeZoneId](QDBusPendingCallWatcher *watcher) {
        ...
        if (timeZone.isValid()) { // Can this ever be false since we did suceeded in setting it?
            timeZoneName = timeZone.displayName(QDateTime::currentDateTime());
        }

        // Show OSD that time zone got changed.
        ...
        msg.setArguments({u"clock"_s,
                          i18nc("New time zone (current time in this time zone)",
                                "%1 (%2)",
                                timeZoneName,
                                QLocale().toString(QTime::currentTime(), QLocale().timeFormat(QLocale::ShortFormat)))});
