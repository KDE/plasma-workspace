...
void PowerDevilRunner::updateSyntaxes()
{
    ...
    RunnerSyntax brightnessSyntax(i18nc("Note this is a KRunner keyword, <> is a placeholder and should be at the end", "screen brightness <percent value>"),
...
void PowerDevilRunner::match(RunnerContext &context)
{
    ...
    if (screenBrightnessResults != -1) {
        ...
        QueryMatch match1(this);
        match1.setType(QueryMatch::ExactMatch);
...
void PowerDevilRunner::addSuspendMatch(int value, QList<QueryMatch> &matches, QueryMatch::Type type)
{
    QueryMatch match(this);
    match.setType(type);
...
void PowerDevilRunner::run(const RunnerContext &context, const QueryMatch &match)
{
    Q_UNUSED(context)
...
bool PowerDevilRunner::matchesRunnerKeywords(const QList<RunnerKeyword> &keywords, QueryMatch::Type &type, const QString &query) const
{
    return std::any_of(keywords.begin(), keywords.end(), [&query, &type](const RunnerKeyword &keyword) {
        ...
        type = exactMatch ? QueryMatch::ExactMatch : QueryMatch::CompletionMatch;
...
{
    RunnerSyntax syntax(keywords.first().translatedTriggerWord, description);
