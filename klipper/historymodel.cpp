...
QVariant HistoryModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DisplayRole: {
        ...
        if (text.isEmpty() && item->allTypes() & HistoryItemType::Image) {
            QSqlQuery query(
                u"SELECT mimetype,data_uuid FROM aux WHERE uuid='%1' AND (mimetype LIKE 'image/%' OR mimetype='application/x-qt-image')"_s.arg(item->uuid()),
                m_db);
            query.exec();
            if (query.exec() && query.isSelect() && query.next()) {
                QSize size = QImageReader(QString(m_dbFolder + u"/data/" + item->uuid() + u'/' + query.value(1).toString())).size();
                return QString(u"▨ " + i18nc("@info:tooltip width x height", "%1x%2", QString::number(size.width()), QString::number(size.height())));
            }
        }
        return QVariant::fromValue(std::move(text));
    }
...
bool HistoryModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    case Qt::DisplayRole: {
        ...
        KIO::del(QUrl::fromLocalFile(m_dbFolder + u"/data/" + item->uuid() + u'/'));
        saveToFile(text.toUtf8(), newUuid, newUuid); // Must be synchronous so the clipboard can be updated immediately
