...
QString LookAndFeelInformation::variant() const
{
    return m_variant;
}

void LookAndFeelInformation::setVariant(const QString &variant)
{
...
void LookAndFeelInformation::refresh()
{
    ...
    for (int i = 0; i < m_model->rowCount(); ++i) {
        ...
        if (index.data(KCMLookandFeel::PluginNameRole) == m_packageId) {
            ...
            setVariant(index.data(KCMLookandFeel::VariantRole).toString());
            return;
        }
    }
}
...
KCMLookandFeel::KCMLookandFeel(QObject *parent, const KPluginMetaData &data)
    ...
    , m_config(new LookAndFeelConfig(m_data->settings(), this))
    ...
    , m_selectedContents(LookAndFeelManager::AppearanceSettings)
{
    ...
    qmlRegisterAnonymousType<LookAndFeelConfig>("", 1);
    ...
    auto handleLookAndFeelPackageChanged = [this]() {
        ...
        const int index = pluginIndex(config()->lookAndFeelPackage());
    ...
    connect(config(), &LookAndFeelConfig::lookAndFeelPackageChanged, handleLookAndFeelPackageChanged);
...
LookAndFeelConfig *KCMLookandFeel::config() const
{
    return m_config;
}
...
static QString determineLookAndFeelVariant(const KPackage::Package &package)
{
...
void KCMLookandFeel::addKPackageToModel(const KPackage::Package &pkg)
{
    ...
    row->setData(determineLookAndFeelVariant(pkg), VariantRole);
...
{
    return config()->isSaveNeeded();
}

void KCMLookandFeel::save()
{
    QString newLnfPackage = config()->lookAndFeelPackage();
    KPackage::Package package = KPackage::PackageLoader::self()->loadPackage(QStringLiteral("Plasma/LookAndFeel"));
