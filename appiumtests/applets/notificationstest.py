...
from notificationstest.jobnotification import TestInterface as JobTestInterface
...
class NotificationsTest(unittest.TestCase):
    ...
    def test_7_job_notification(self) -> None:
        """
        https://bugs.kde.org/show_bug.cgi?id=502001
        """
        proc = subprocess.Popen(["python3", os.path.join(os.path.dirname(os.path.abspath(__file__)), "notificationstest", "jobnotification.py")], stdout=sys.stderr, stderr=sys.stderr)
        ...
        session_bus.call_sync(JobTestInterface.BUS_NAME, JobTestInterface.OBJECT_PATH, JobTestInterface.BUS_NAME, "SetProcessedAmount", GLib.Variant("(t)", [1024 * 1024 * 2]), None, Gio.DBusSendMessageFlags.NONE, 1000)
        ...
        self.driver.find_element(AppiumBy.NAME, "2.0 MiB of 10.0 MiB")

        # Change progress to 50%
        session_bus.call_sync(JobTestInterface.BUS_NAME, JobTestInterface.OBJECT_PATH, JobTestInterface.BUS_NAME, "SetProcessedAmount", GLib.Variant("(t)", [1024 * 1024 * 5]), None, Gio.DBusSendMessageFlags.NONE, 1000)
        ...
        self.driver.find_element(AppiumBy.NAME, "5.0 MiB of 10.0 MiB")

        # Change progress to 100%. The job should emit finished signal.
        session_bus.call_sync(JobTestInterface.BUS_NAME, JobTestInterface.OBJECT_PATH, JobTestInterface.BUS_NAME, "SetProcessedAmount", GLib.Variant("(t)", [1024 * 1024 * 10]), None, Gio.DBusSendMessageFlags.NONE, 1000)
