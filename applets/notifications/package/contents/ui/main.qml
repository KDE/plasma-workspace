...
Item {
    ...
    readonly property int effectiveStatus: historyModel.activeJobsCount > 0 || historyModel.unreadNotificationsCount > 0 || Globals.inhibited ? PlasmaCore.Types.ActiveStatus : PlasmaCore.Types.PassiveStatus
    ...
    Plasmoid.toolTipSubText: {
        ...
        if (!NotificationManager.Server.valid) {
            ...
            if (Globals.inhibited) {
                ...
                if (inhibitedUntilValid && inhibitedUntil.getTime() - Date.now() < 100 * 24 * 60 * 60 * 1000) /* 100 days*/ {
                    lines.push(i18n("Do not disturb until %1; middle-click to exit now", KCoreAddons.Format.formatRelativeDateTime(inhibitedUntil, Locale.ShortFormat)));
                } else {
    ...
    NotificationManager.Notifications {
        ...
        urgencies: {
            var urgencies = NotificationManager.Notifications.CriticalUrgency | NotificationManager.Notifications.NormalUrgency;
