...
Item {
    ...
    // The job keeps the readings, so a chart opened late still has the whole job.
    ...
    // Rounded up to a one, a two or a five: readings wander, and an axis following the highest of
    // them exactly redraws the whole line at a new height on every reading.
    readonly property real scaleTop: {
        const peak = root.maxSpeed * 1.05;
        if (!(peak > 0)) {
            return 1;
        }
        // Binary steps, so formatByteSize writes round labels beside them.
    ...
    // Taken from the readings, not worked out again from the bytes: the two round differently and
    // the line swims under the edge meant to follow it.
    readonly property real reached: chartValues.length > 1
        ? (speedHistory.length - 1) / (chartValues.length - 1)
        : 0

    // Always a hundred points: the chart anti-aliases against its own diagonal, so one that grows
    // narrow when there is little to show draws that little badly. The points past where the job
    // has got repeat the last reading and are covered rather than drawn.
        ...
        sourceComponent: Item {
            ...
            Item {
                ...
                Charts.LineChart {
                    ...
                    xRange.from: 0
                    // The range is the count, not the last index: the chart draws that many points
                    // and spaces them by width/(count - 1), so a range short by one drops the last
                    // reading and spreads the rest wider than the window expects.
