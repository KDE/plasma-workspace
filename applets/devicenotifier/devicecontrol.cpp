...
QVariant DeviceControl::data(const QModelIndex &index, int role) const
{
    ...
    case Size:
        return deviceInfo.spaceInfo ? deviceInfo.spaceInfo->getFullSize() : QVariant();
    case FreeSpace:
        return deviceInfo.spaceInfo ? deviceInfo.spaceInfo->getFreeSize() : QVariant();
    case SizeText: {
        std::optional<double> size = deviceInfo.spaceInfo ? deviceInfo.spaceInfo->getFullSize() : std::optional<double>();
        return size.has_value() ? KFormat().formatByteSize(size.value()) : QVariant();
    }
    case FreeSpaceText: {
        std::optional<double> freeSpace = deviceInfo.spaceInfo ? deviceInfo.spaceInfo->getFreeSize() : std::optional<double>();
