...
PlasmaExtras.ExpandableListItem {
    ...
    ListView.onRemove: SequentialAnimation {
        ...
        PropertyAction {
            target: deviceItem
            property: "contextualActionsModel"
            value: []
        }
        PropertyAction {
            target: deviceItem
            property: "icon"
            value: statusSource.lastIcon
        }
        PropertyAction {
            target: deviceItem
            property: "title"
            value: statusSource.lastDescription
        }
        PropertyAction {
            target: deviceItem
            property: "subtitle"
            value: statusSource.lastMessage
        }
        PauseAnimation {
            duration: messageHighlightAnimator.duration
        }
        // Otherwise the last message will show again when this device reappears
        ScriptAction {
            script: statusSource.clearMessage()
        }
    ...
    Instantiator {
        ...
        onObjectRemoved: (index, object) => {
            deviceItem.contextualActionsModel = Array.prototype.slice.call(deviceItem.contextualActionsModel).filter(action => action !== object);
        }
    }
