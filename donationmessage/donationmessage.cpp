...
static const QUrl DONATION_URL = QUrl(u"https://kde.org/community/donations/?app=plasma-donation-request-notification"_s);
    ...
    : KDEDModule(parent)
{
    ...
    const int currentDay = currentDate.day();
    ...
    if (systemInstallDate.isEmpty()) {
        const QString systemInstallDateString = QStringLiteral("%1-%2-%3").arg(currentYear).arg(currentMonth).arg(currentDay);
    ...
    const QList dateComponents = systemInstallDate.split(u"-"_s);
    const QDate installedDate = QDate(dateComponents[0].toInt(), dateComponents[1].toInt(), dateComponents[2].toInt());

    if (installedDate.daysTo(currentDate) < MIN_INSTALLED_DAYS_BEFORE_ASKING) {
