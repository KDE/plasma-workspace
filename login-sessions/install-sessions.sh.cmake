...
# https://github.com/systemd/systemd/issues/31523
if [ $(basename @CMAKE_INSTALL_PREFIX@) != "usr" ]; then
    echo "The basename of your install prefix must be 'usr' to be compatible with the systemd-sysext setup."
    echo "This is a limitation of the systemd-sysext implementation."
    echo "If you are using kdesrc-build or kde-builder, please set the install-dir option to paths like ~/kde/usr, /usr/local/kde/usr, etc."
    exit 1
fi

if [ "$(id -u)" != "0" ]; then
  install @CMAKE_BINARY_DIR@/prefix.sh @CMAKE_INSTALL_FULL_LIBEXECDIR@/plasma-dev-prefix.sh
  install @CMAKE_CURRENT_BINARY_DIR@/startplasma-dev.sh --target-directory=@CMAKE_INSTALL_FULL_LIBEXECDIR@
  ...
  install --mode=755 @CMAKE_CURRENT_BINARY_DIR@/run-unmerged --target-directory=@CMAKE_INSTALL_FULL_BINDIR@
  install --mode=755 @CMAKE_CURRENT_SOURCE_DIR@/run-unmerged-script --target-directory=@CMAKE_INSTALL_FULL_LIBEXECDIR@
