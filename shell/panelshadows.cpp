...
public:
    ...
    QHash<QWindow *, QMargins> m_margins;
...
void PanelShadows::addWindow(QWindow *window, KSvg::FrameSvg::EnabledBorders enabledBorders, const QMargins &margins)
{
    if (!window) {
        return;
    }
...
void PanelShadows::Private::updateShadow(QWindow *window, KSvg::FrameSvg::EnabledBorders enabledBorders)
{
    ...
    QMargins padding;
    QMargins extraPadding = m_margins.value(window);
