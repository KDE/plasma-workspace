...
void PanelView::positionPanel()
{
    ...
    if (m_layerWindow) {
        ...
        if (formFactor() == Plasma::Types::Horizontal) {
            ...
            // Vertical panels are always limited to dodge horizontal ones
            margins.setTop(std::max(margins.top(), int(containment()->availableRelativeScreenRect().top())));
            margins.setBottom(std::max(margins.bottom(), screenToFollow()->geometry().bottom() - int(containment()->availableRelativeScreenRect().bottom())));
...
void PanelView::resizePanel()
{
    ...
    // When we are using layershell, horizontal sizing is completely managed by the compositor
    // for horizontal panels (and height for vertical panels)
    if (m_layerWindow && m_lengthMode == PanelView::LengthMode::FillAvailable) {
        if (formFactor() == Plasma::Types::Vertical) {
