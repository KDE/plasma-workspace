...
class PlasmaWindow : public QObject, public QtWayland::org_kde_plasma_window
{
    Q_OBJECT
public:
    PlasmaWindow(QObject *parent, ::org_kde_plasma_window *id)
        : QObject(parent)
...
protected:
    ...
    {
        const bool newActive = flags & state::state_active;
        if (newActive != active) {
            active = newActive;
...
public:
    ...
    {
        Q_UNUSED(id)
        Q_EMIT windowCreated(new PlasmaWindow(this, get_window_by_uuid(uuid)));
    }
