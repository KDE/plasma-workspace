...
Job::TextAmount JobPrivate::textAmount() const
{
    if (!m_errorText.isEmpty() || !m_infoMessage.isEmpty()) {
        // Those are the text, and neither carries an amount.
        return Job::TextAmountNone;
    }

    if (m_totalFiles > 1) {
        return Job::TextAmountFiles;
    }

    if (m_totalItems > 1) {
        return Job::TextAmountItems;
    }
...
QString JobPrivate::text() const
{
    ...
    const Job::TextAmount amount = textAmount();

    if (amount == Job::TextAmountFiles) {
    ...
    } else if (amount == Job::TextAmountItems) {
