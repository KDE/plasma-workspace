...
Portal &Portal::self()
{
    static auto *s_self = new Portal(qApp);
    return *s_self;
}

uint Portal::portalIdMask()
{
    return 1u << 31;
}

bool Portal::isPortalNotificationId(uint notificationId)
{
    return notificationId & portalIdMask();
}
...
bool Portal::init()
{
    ...
    m_valid = true;
    qCritical() << "HALLO PORTEL";
    return true;
}

void Portal::removeNotification(uint notificationId)
{
    m_idMap.remove(notificationId);
}

void Portal::invokeAction(uint notificationId, const QString &actionId, const QVariant &target, const QString &response, QWindow *window)
{
    ...
    auto activateWithPlatformData = [this, appId, portalNotificationId, actionId, target, response](const QVariantMap &platformData) {
        if (constexpr QLatin1String appPrefix{"app."}; actionId.startsWith(appPrefix)) {
            ...
            qCritical() << "INWOUK DBUS APP" << appId << portalNotificationId << actionId << appPath << params << platformData;
            ...
            qCritical() << "EMIT NOTI" << appId << portalNotificationId << actionId << params << platformData;
...
void Portal::AddNotification(const QString &appId, const QString &portalNotificationId, const QVariantMap &props)
{
    qCritical() << "ADD NOTI" << appId << portalNotificationId << props;
    ...
    if (!wasReplaced) {
        ...
        if (!m_highestNotificationId) {
            m_highestNotificationId = portalIdMask() + 1;
        }
    ...
    if (const QString markupBody = props.value(QStringLiteral("markup-body")).toString(); !markupBody.isEmpty()) {
        ...
        // version 2 has an explicit body-markup but version 1 does not.
...
void Portal::RemoveNotification(const QString &appId, const QString &portalNotificationId)
{
    const auto idPair = QPair(appId, portalNotificationId);
    const auto notificationId = m_idMap.key(idPair);

    qCritical() << "RM NOTI" << appId << portalNotificationId << "=" << notificationId;
