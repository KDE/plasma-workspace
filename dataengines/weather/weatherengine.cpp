...
void WeatherEngine::removeIonSource(const QString& source)
{
    ...
    if (ion) {
        ...
        if (it == m_ionUsage.end()) {
            qWarning() << "Removing ion source without being added before:" << source;
        } else {
            // no longer used?
            if (it.value() <= 1) {
                // forget about it
                ...
                qDebug() << "Ion no longer used as source:" << ionName;
            } else {
                --(it.value());
            }
        }
    } else {
        qWarning() << "Could not find ion to remove source for:" << source;
    }
}
...
bool WeatherEngine::sourceRequestEvent(const QString &source)
{
    ...
    if (!ion) {
        qWarning() << "Could not find ion to request source for:" << source;
        return false;
    }
    ...
    if (it == m_ionUsage.end()) {
        ...
        qDebug() << "Ion now used as source:" << ionName;
    } else {
        ++(*it);
    }
    ...
    qDebug() << "sourceRequestEvent(): Network is: " << m_networkConfigurationManager.isOnline();
...
{
    qDebug() << "updateSourceEvent(): Network is: " << m_networkConfigurationManager.isOnline();
    ...
    if (!ion) {
        qWarning() << "Could not find ion to update source for:" << source;
        return false;
    }
...
{
    if (isOnline) {
        qDebug() << "starting m_reconnectTimer";
...
void WeatherEngine::startReconnect()
{
    for(QHash<QString, int>::ConstIterator it = m_ionUsage.constBegin(); it != m_ionUsage.constEnd(); ++it) {
        ...
        if (ion) {
            qDebug() << "Resetting ion" << ion;
            ion->reset();
        } else {
            qWarning() << "Could not find ion to reset:" << ionName;
        }
    }
}
...
void WeatherEngine::forceUpdate(IonInterface *ion, const QString &source)
{
    ...
    if (container) {
        ...
        qWarning() << "innexplicable failure of" << source;
    }
}
